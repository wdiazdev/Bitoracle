...
export const MarketTable = ({ page, itemsPerPage, search, cryptoData, setPage }) => {
  ...
  const handleSearch = () => {
    ...
    return cryptoData.filter(
      (crypto) => crypto.name.toLowerCase().includes(search) || crypto.symbol.toLowerCase().includes(search),
    )
  }
            ...
            return (
              <tbody key={coin.id}>
                <tr className="table--body--rows">
                  ...
                  <td className="col--hr--change" style={priceChange > 0 ? { color: "#7CFC00" } : { color: "#DC0000" }}>
                    {priceChange}%
                  </td>
